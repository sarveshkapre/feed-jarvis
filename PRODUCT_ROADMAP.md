...
- Best-in-market references (untrusted web):
  ...
  - Inoreader docs/blog: rules/filters automation and recurring retry behavior for feed fetcher (`inoreader.com`).
...
- Parity gap map:
  - Missing: Studio OPML import/export for saved feed sets; browser E2E for critical Studio flow.
  - Weak: transient fetch failure handling; input URL validation in pasted JSON workflow.
  - Parity: local feed sets/rules, item filters, metadata-rich exports, agent feed layouts.
...
- P1: Studio OPML import/export for saved feed sets (local-only interoperability).
- P1: Browser-level Studio E2E coverage for critical flow (fetch -> generate -> export) in CI.
- P1: Studio live over-max-char warnings while editing drafts + one-click trim suggestion.
...
## Risks And Blockers
- Risk: multi-feed fetches can fail noisily on transient upstream outages without retries.
- Risk: pasted JSON feeds may include invalid or non-web URLs, reducing output quality and breaking downstream links.
- Risk: large pending UX work in `web/app.js` carries rising maintenance cost until modularized.
- Active blockers: none currently.

## Next Cycle Goals
- Complete remaining P1 parity items (Studio OPML import/export and Studio E2E critical flow coverage).
